...
def registry() -> InstanceRegistry:
    ...
    for i in range(3):
        r.register(ModelInstance(
            instance_id=f"i{i+1}", address=f"http://gpu-{i+1}:8000",
            model="test", engine_type="ollama",
        ))
    return r
...
class TestScaleUp:
    def test_scale_up_triggered(self, scaler: AutoScaler) -> None:
        scaler._metrics.get_summary.return_value = {
            "queue_depth": 15, "per_instance": {"i1": {"request_count": 5}, "i2": {"request_count": 5}, "i3": {"request_count": 5}},
        }
    ...
    def test_no_scale_up_at_max(self, scaler: AutoScaler, registry: InstanceRegistry) -> None:
        ...
        scaler._metrics.get_summary.return_value = {
            "queue_depth": 15, "per_instance": {"i1": {"request_count": 5}},
        }
...
class TestScaleDown:
    def test_scale_down_triggered(self, scaler: AutoScaler, registry: InstanceRegistry) -> None:
        scaler._metrics.get_summary.return_value = {
            "queue_depth": 0, "per_instance": {"i1": {"request_count": 0}},
        }
        scaler._last_scale_time = 0
...
class TestNoAction:
    def test_normal_operation(self, scaler: AutoScaler) -> None:
        scaler._metrics.get_summary.return_value = {
            "queue_depth": 3, "per_instance": {"i1": {"request_count": 2}},
        }
        decision = scaler.evaluate()
        assert decision.action == "none"
