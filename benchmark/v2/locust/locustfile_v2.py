...
用法:
    ...
    uv run locust -f benchmark/v2/locust/locustfile_v2.py --host=http://localhost:8000
    # 通过 Dispatcher

支持 Stair load (自定义 ramp_up / soak 模式)。
"""
...
PROMPTS = [
    ("Hi", 10, 0.5),  # 50% 极短
    ("Hello, how are you?", 20, 0.3),  # 30% 短
    ("Explain quantum computing in one sentence", 50, 0.15),  # 15% 中
    ("Explain quantum computing and its real-world applications", 200, 0.05),  # 5% 长
]
